...
const { data: timingsResponse, status, error } = await useFetch(endpoint, {
  watch: [endpoint],
  immediate: true,
})

const timings = computed(() => timingsResponse.value)
...
<template>
  ...
  <div v-else-if="status === 'pending'" class="border rounded p-5">
    <LoadingState />
  </div>

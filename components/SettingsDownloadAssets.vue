...
const downloadedSurahs = ref({})
const downloadedAzkar = ref({})

// Initialize from localStorage on client
onMounted(() => {
  if (import.meta.client) {
    const storedSurahs = localStorage.getItem('downloadedSurahs')
    const storedAzkar = localStorage.getItem('downloadedAzkar')
    if (storedSurahs) downloadedSurahs.value = JSON.parse(storedSurahs)
    if (storedAzkar) downloadedAzkar.value = JSON.parse(storedAzkar)
...
watch(downloadedSurahs, (newValue) => {
  if (import.meta.client) {
    localStorage.setItem('downloadedSurahs', JSON.stringify(newValue))
  }
}, { deep: true })

watch(downloadedAzkar, (newValue) => {
  if (import.meta.client) {
    localStorage.setItem('downloadedAzkar', JSON.stringify(newValue))
  }
}, { deep: true })
...
const downloadSurah = async (surah) => {
  ...
  const data = await $fetch(`https://api.alquran.cloud/v1/surah/${surah.id}`)
  downloadedSurahs.value[surah.id] = data
}

const downloadAzkarCategory = async (category) => {
  downloadState.value.currentItem = `${category.name || category.id}`
  const data = await $fetch(`/data/azkar/${category.id}.json`)
  downloadedAzkar.value[category.id] = data
}

...
export const useQuranStore = defineStore('audio', () => {
  ...
  if (import.meta.client) {
    const stored = localStorage.getItem('currentReciter')
    if (stored) currentReciter.value = stored
  }

  // Watch for changes and persist
  watch(currentReciter, (newValue) => {
    if (import.meta.client) {
      localStorage.setItem('currentReciter', newValue)
    }
  })

    ...
    <Page v-if="surah">
      <div class="mx-auto flex w-full max-w-2xl flex-col">
        ...
        <article class="rounded-3xl bg-card px-4 py-6 text-card-foreground shadow-sm sm:px-7 sm:py-8">
          <p v-if="surahId != 9" class="mb-6 text-center text-2xl leading-quran text-primary font-quran sm:text-3xl">
          ...
          <div class="text-justify font-quran [text-align-last:center] [text-justify:inter-word]">
            <template v-for="(ayah, index) in ayat" :key="ayah.number">
              <span
                ...
                class="ayah box-decoration-clone cursor-pointer rounded-md text-2xl leading-quran sm:text-3xl"
              ...
              >
                <span
                  class="rounded-full bg-muted px-3 py-1 font-sans text-xs whitespace-nowrap text-muted-foreground"
                  >{{ toArabicNumerals(ayah.page) }}</span
                >
              </div>
            </template>
          </div>

...
const canShare = computed(() => import.meta.client && typeof navigator !== 'undefined' && typeof navigator.share === 'function')

// Set SEO meta when station is available
watch(station, (data) => {
  if (data) {
    useSeoMeta({
      title: `${data.name} - رفيق`,
      ogTitle: `${data.name} - رفيق`,
      description: `استمع إلى ${data.name} بث مباشر`,
      ogDescription: `استمع إلى ${data.name} بث مباشر`,
      keywords: `إذاعة, راديو, قرآن, ${data.name}, بث مباشر`,
    })
  }
}, { immediate: true })
...
<template>
  ...
  <Page v-else class="radio-station-page">
    ...
    <div v-if="!station" class="not-found-container">
      ...
      <NuxtLink class="back-btn" to="/radio">
    ...
    <div v-else class="player-wrapper">
      ...
      <div class="player-header">
        <NuxtLink class="nav-btn" to="/radio">
          <IconArrowLeft size="1.25rem" />
          <span>العودة</span>
        </NuxtLink>

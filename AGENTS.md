...
## Design Context
Key design constraints: colors flow through the design tokens in `src/shared/styles/main.css` (never hardcoded — primary and background are user-themeable at runtime), all sizes in `rem` (user font scaling), Arabic RTL-first, Kitab font for Quran text only.
